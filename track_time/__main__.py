...
if __name__ == "__main__":
    ...
    group.add_argument(
        "-n", "--project_name", help="The project this task is associated to", default="Other"
    )
    group.add_argument("-p", "--list_projects", help="List previous projects", action="store_true")
    group.add_argument(
        "-u", "--count_up", help="Time up for 0 till asked to stop", action="store_true"
    )
    group.add_argument("-o", "--open", help="Open the database", action="store_true")
    parser.add_argument(
        "-s", "--subscribe", help="Count down the current timer", action="store_true"
    )
    parser.add_argument(
        "-l", "--length", type=int, help="The length of the task in minutes", default=20
    )
    ...
    if args.list_projects:
        ...
        if not project_name and not time_spent:
            print(
                "ERROR: For event at {} no time or project name has been given and the event was less that 1 minute".format(
                    get_as_nearest_time_string(start_time, 1)
                )
            )
            exit()
        elif not project_name:
            print(
                "ERROR: {},{} event not saved since no project name given".format(
                    get_as_nearest_time_string(start_time, 1), time_spent
                )
            )
            exit()
        elif not time_spent:
            print(
                "ERROR: Event {},0,{} not saved since less than a minute long".format(
                    get_as_nearest_time_string(start_time, 1), project_name
                )
            )
            exit()
    ...
    # Start a new task
        ...
        end_time_string = get_as_nearest_time_string(
            start_time + timedelta(minutes=minutes_to_wait), round_to_time_interval
        )
        ...
        if args.subscribe:
            ...
            secs_past = 0
            while (
                datetime.now() < end_time or secs_past > 1000000
            ):  # TODO:Poor maximum subscribe time
                ...
                system(
                    ...
                    + str(chosen_project)
                    + ":"
                    + str(get_timedelta_minute_string(end_time - cur_time))
                )
